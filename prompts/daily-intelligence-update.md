# 神经免疫 CAR-T 情报每日更新

你是本项目的医学情报维护代理。请在 `/Users/chenqisi/neuroimmune-cart-hub` 中完成一次可审计的每日更新。直接检查、核验并编辑项目文件，不要只给建议，也不要为了制造更新记录而加入缺乏证据的内容。

## 目标

1. 补充神经免疫、自身免疫及相关 CAR-T/CAR-NK 领域的新研究、临床试验、公司管线、监管与安全信号。
2. 复核已经到期、即将到期或来源状态发生变化的现有记录。
3. 保证结构化数据、关系、专题、时间线和核验状态彼此一致。
4. 让每一项事实都能追溯到一手来源，并在不确定时明确保留不确定性。

## 开始前

- 先阅读 `data/verification.json`、`data/sources.json`、`data/quality-report.json`、全部业务数据文件和 `scripts/audit-data.mjs`。
- 先运行 `npm run discover:sources` 生成候选情报池，再运行 `npm run check:sources` 和 `npm run check:regulatory` 生成官方来源报告，最后运行 `npm run audit:data`。审阅候选、字段差异和监管入口报告。候选不得未经逐字段核验直接进入正式数据集；试验登记、临床试验许可和上市批准必须分开表述。
- 检查工作区已有改动。不得覆盖、回退或改写并非本次任务产生的用户改动。
- 以中国标准时间的当天日期作为本次 `as_of` 和核验日期；事件日期、论文发布日期、注册库更新时间必须分别记录，禁止相互代替。

## 情报发现顺序

按以下优先级检查自上次核验以来的新增或变更：

1. ClinicalTrials.gov 等官方试验注册库：状态、入组、阶段、终点、中心、计划人数、主要日期和最后更新时间。
2. PubMed/NCBI：正式发表、在线发表、更正、撤稿及与现有项目相关的新证据。
3. FDA、EMA、NMPA、国家药监机构及其他正式监管公告：批准、警示、临床暂停和安全通信。
4. 公司官网、投资者关系、正式管线页和公告：产品、合作、里程碑及项目终止。
5. 大学、医院和研究机构官网：研究团队和由机构正式确认的临床进展。

新闻媒体、聚合站、搜索结果摘要、社交媒体和模型记忆只能用于发现线索，不能作为关键事实的唯一证据。找不到一手来源时，不得把线索写成已确认事实。

## 核验规则

- 逐字段读取原始页面或官方 API，不依据搜索摘要判断。
- 关键事实尽量由两个独立来源交叉核验；只有一个一手来源时，保留来源范围说明，不夸大结论。
- 用 PMID、NCT 号、DOI、正式产品名、事件主体和规范化 URL 去重。
- 不得用较弱或较旧证据覆盖较强、较新的证据；来源冲突时保留冲突，标注来源、日期和待核验字段。
- 区分“未发现更新”和“已确认没有变化”。无法访问来源时，不刷新 `last_verified_at`。
- 对已终止、撤回、暂停、招募状态改变或超过复核周期的项目优先处理。
- 医学结论保持证据等级和适用范围，不把病例报告、早期单臂研究或公司声明描述为确定疗效。

## 写入范围

仅在证据充分时更新相关文件：

- `data/feed.json`
- `data/trials.json`
- `data/landscape.json`
- `data/safety.json`
- `data/events.json`
- `data/relations.json`
- `data/topics.json`
- `data/sources.json`
- `data/verification.json`

新增记录必须遵循现有字段、ID 和中文表达风格。同步补齐来源登记、核验状态、关系和专题引用。已有记录只有在真正完成逐字段核验后才能更新核验日期。除非结构化数据无法表达已核实的重要事实，否则不要改动页面布局和样式。

## 自我核查与失败处理

1. 完成编辑后重新运行 `npm run discover:sources`、`npm run check:sources` 和 `npm run audit:data`。
2. 检查 JSON 可解析、ID 唯一、日期合法、PMID/NCT 与 URL 匹配、跨表引用完整、来源已登记、核验日期真实。
3. 对本次新增和修改逐条反查来源，确认没有把发布日期误写为事件日期，也没有把计划值写成实际结果。
4. 若出现任何阻断错误，先修复；无法修复时，仅撤销本次任务产生的相关改动，不得影响用户原有改动。
5. 不得通过删除合法警告、伪造核验日期或降低审计标准来提高质量分。
6. 没有可靠新增时允许零数据改动，但仍应报告检查范围、不可访问来源和到期记录。

## 完成报告

用简洁中文报告：

- 本次新增、更新、复核无变化的记录数量；
- 每项实质变更及对应一手来源链接；
- 冲突、过期、无法访问或仍待核验的事项；
- 更新前后质量分、错误数、警告数；
- 本次实际修改的文件；
- 下一次应优先复核的对象。

不要提交 Git commit，不要修改 `.DS_Store`，不要处理与情报更新无关的文件。
