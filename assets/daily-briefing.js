(() => {
  ...
  style.textContent = `.briefing{padding:24px 0;border-bottom:1px solid var(--border)}.briefing-head{display:flex;justify-content:space-between;gap:16px;align-items:end;margin-bottom:13px}.briefing-head h2{font-size:24px}.briefing-head p{max-width:620px;color:var(--text2);font-size:12px}.briefing-grid{display:grid;grid-template-columns:1.2fr 1fr 1fr;gap:9px}.briefing-panel{border:1px solid var(--border);border-radius:8px;background:var(--surface);padding:15px}.briefing-panel h3{font-size:13px;margin-bottom:9px}.briefing-item{padding:8px 0;border-bottom:1px solid var(--border)}.briefing-item:last-child{border:0}.briefing-item a{text-decoration:none;font-weight:700;font-size:12px}.briefing-item a:hover{color:var(--accent)}.briefing-item p{margin-top:3px;color:var(--text2);font-size:10px}.briefing-meta{color:var(--text3);font:9px var(--mono)}.briefing-risk{color:#936100}.briefing-foot{margin-top:9px;color:var(--text3);font-size:10px}@media(max-width:900px){.briefing-grid{grid-template-columns:1fr}.briefing-head{align-items:start;flex-direction:column}}`;
  ...
  fetch("data/daily-briefing.json").then((response) => response.json()).then((briefing) => {
    ...
    const risks = briefing.risks.map((item) => `<div class="briefing-item"><a class="briefing-risk" href="quality.html">${esc(item.message)}</a><p>${esc(item.record_type)} · ${esc(item.record_id)}</p></div>`).join("");
    section.innerHTML = `<div class="briefing-head"><div><div class="eyebrow">DAILY BRIEFING</div><h2>今日决策摘要</h2></div><p>${esc(briefing.editorial_note)} 当前正式记录 ${briefing.snapshot.formal_records} 条，外部核验 ${briefing.snapshot.externally_verified} 条。</p></div><div class="briefing-grid"><article class="briefing-panel"><h3>已核验动态</h3>${updates || "暂无新增动态"}</article><article class="briefing-panel"><h3>重点试验</h3>${trials || "暂无重点试验"}</article><article class="briefing-panel"><h3>风险与待复核</h3>${risks || "当前无风险提示"}</article></div><div class="briefing-foot">生成于 ${new Date(briefing.generated_at).toLocaleString("zh-CN")} · 质量 ${briefing.snapshot.quality_score} · 候选池 ${briefing.snapshot.candidate_items}</div>`;
